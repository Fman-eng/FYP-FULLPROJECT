...
% - Famp : Matrix (6xnBodies)xlength(w) of exciation forces (amplitude)
% - Fphi : Matrix (6xnBodies)xlength(w) of exciation forces (phase)
% - Ainf : Infinite added mass coefficients
% values)
%
...
%
function [A,B,Fe,Famp,Fphi,Ainf]=runNemoh(w, dir, depth, nomrep, nemohdir)
% Preparation du calcul
fprintf('%s',fullfile(nomrep,'ID.dat'));
fid=fopen(fullfile(nomrep,'ID.dat'));
%fid=fopen('ID.dat');
line=fgetl(fid);
rep=fscanf(fid,'%s',1);
fclose('all');
...
if l == 1
    ...
    fprintf('\n------ Starting NEMOH ----------- \n');
    system(fullfile(nemohdir,'preProcessor.exe'));
    fprintf('------ Solving BVPs ------------- \n');
    system(fullfile(nemohdir,'Solver.exe'));
    fprintf('------ Postprocessing results --- \n');
    system(fullfile(nemohdir,'postProcessor.exe'));
end
...
status=fclose(fid);
fid=fopen([rep,filesep,'results',filesep,'IRF.tec'],'r');
ligne=fgetl(fid);
for i=1:6*nBodies
    ligne=fgetl(fid);
end;
for i=1:nBodies*6
    ...
    for k=1:ntimestep
        ligne=fscanf(fid,'%f',1+12*nBodies);
        for j=1:6*nBodies
            AinfChunk(i,j,k)=ligne(2*j);
            IRF(i,j,k)=ligne(2*j+1);
        end;
        ligne=fgetl(fid);
    end;
end;
