...
%
function [Mass,Inertia,KH,XB,YB,ZB]=Mesh(nBodies,n,X,tX,CG,nfobj,nomrep)
rho=1025;
g=9.81;
% status=close('all');
...
%   Creation des fichiers de calcul du maillage
    ...
    fprintf(fid,'1 \n '); % 0 if not symmetric, 1 if symmetric
    fprintf(fid,'%f 0. \n ',tX(c));
    fprintf(fid,'%f %f %f \n',CG(c,:));
    fprintf(fid,'%g \n 2 \n 0. \n 1.\n',nfobj(c));
    fprintf(fid,'%f \n %f \n',[rho g]);
...
%   Raffinement automatique du maillage et calculs hydrostatiques
    ...
    if l == 1
        ...
        system([fullfile(pwd,'..','NEMOH_files','Mesh','Mesh.exe'), ' >' ,fullfile(pwd,'..','NEMOH_files','Mesh','Mesh.log')]);
    end
...
%     else
%         fprintf(fid,[nomrep,'\\mesh\\Output.mar      ! Name of mesh file\n']);
%     end;
